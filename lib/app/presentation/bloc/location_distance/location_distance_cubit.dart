...
class LocationDistanceCubit extends AppBaseCubit<LocationDistanceState> {
  ...
  void getDistance([String? latitude, String? longitude]) async {
    try {
      ...
      timer = Timer.periodic(const Duration(seconds: 15), (Timer t) => _updateLocation(targetLatitude, targetLongitude));
  ...
  void _updateLocation(double? latitude, double? longitude) async {
    try {
      ...
      if (distanceMeters < 1000) {
        final processedState = LocationDistanceState.processed('${distanceMeters.toStringAsFixed(2)} m');
        emitSafeState(processedState);
      } else {
        final processedState = LocationDistanceState.processed('${distanceMeters.toStringAsFixed(2)} Km');
        emitSafeState(processedState);
      }
    } on Exception catch (e, st) {
      $logger.error('[LocationDistanceCubit]', e, st);
...
extension GeoLocationStateApi on LocationDistanceState {
  ...
  bool _validateRange() {
    ...
    if (isSuccess) {
      ...
      final match = regex.stringMatch(distance.valueOrEmpty);
      if (match.isNotNull) {
        final range = double.parse(match!);
        return (range < 300);
      }
      return false;
    }
