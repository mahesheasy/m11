...
class RequestExtraCansScrn extends StatefulWidget {
  const RequestExtraCansScrn({super.key});

  @override
  State<RequestExtraCansScrn> createState() => _RequestExtraCansScrnState();
}

class _RequestExtraCansScrnState extends State<RequestExtraCansScrn> {
  double cans = 0;
  ...
  Widget build(BuildContext context) {
    return AppScaffoldWidget(
      title: 'Request Extra Cans',
      headerWidget: const Positioned(
        top: kTextTabBarHeight + 32,
        left: 12,
        child: PageHeaderWidget(title: 'Request Extra Cans'),
      ),
      child: SpacedColumn(
        ...
        children: [
          ...
          BlocConsumer<RequestExtraCansCubit, RequestExtraCansCubitState>(
            listener: (context, state) {
              state.handle(
                context,
                success: (data) => context.showSnackbar(
                    'Can Request created Successfully',
              ...
              onPressed: () {
                if (cans == 0) {
                  context.showSnackbar(
                      'Cans cant be Zero', AppSnackBarType.info);
                } else {
